...
export const useProjectStore = defineStore('project', () => {
  ...
  const pendingProjectFiles = new Set<string>()
  let projectFileCreationQueue = Promise.resolve()
  ...
  function _processTreeEvent(type: string, change: TreeChange): void {
    ...
    switch (type) {
      case 'add': {
        ...
        if (
          isMarkdown &&
          ((newFileNameCache.value && pathname === newFileNameCache.value) ||
            pendingProjectFiles.has(pathname))
        ) {
          const fileState = getFileStateFromData(data as Record<string, unknown>)
          editorStore.UPDATE_CURRENT_FILE(fileState)
          if (pathname === newFileNameCache.value) {
            newFileNameCache.value = ''
          }
          pendingProjectFiles.delete(pathname)
        }
        break
      }
      case 'unlink':
  ...
  function ASK_FOR_OPEN_PROJECT_PATH(pathname: string): void {
    window.electron.ipcRenderer.send('mt::open-project-in-sidebar', pathname)
  }

  function REFRESH_PROJECT(): void {
    const pathname = projectTree.value?.pathname
    if (!pathname) return

    // Drop the current snapshot before the main process restarts its watcher.
    // Chokidar then replays a complete initial add/addDir stream into a clean tree.
    projectTree.value = createProjectRoot(pathname)
    pendingTreeEvents.value = []
    window.electron.ipcRenderer.send('mt::refresh-project-in-sidebar')
  }

  function CREATE_PROJECT_FILE(markdown: string = ''): Promise<void> {
    const rootDirectory = projectTree.value?.pathname
    if (!rootDirectory) return Promise.resolve()

    const operation = projectFileCreationQueue.then(async() => {
      let index = 1
      let pathname = window.path.join(rootDirectory, `Untitled-${index}.md`)
      while (await window.fileUtils.pathExists(pathname)) {
        index += 1
        pathname = window.path.join(rootDirectory, `Untitled-${index}.md`)
      }

      pendingProjectFiles.add(pathname)
      try {
        await window.fileUtils.outputFile(pathname, markdown)
      } catch (err) {
        pendingProjectFiles.delete(pathname)
        notice.notify({
    ...
    projectFileCreationQueue = operation.catch(() => undefined)
    return operation
  }
