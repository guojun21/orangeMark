...
class App {
  ...
  ready = (): void => {
    ...
    const lastOpenedFolderInfo = lastOpenedFolder
      ? normalizeMarkdownPath(lastOpenedFolder)
      : null
    const lastProjectFolder = lastOpenedFolderInfo?.isDir
      ? lastOpenedFolderInfo.path
      : null
    ...
    const createWindow = (): void => {
      if (isRestorePathway) {
        // A restorable editor window must always have a real project. When the
        // last project disappeared, start with the project welcome screen and
        // leave the old recovery buffers untouched instead of restoring them
        // into a pathless editor.
        ...
        if (bufferStoreList.length === 0) {
          this._createEditorWindow(lastProjectFolder)
          return
        }
    ...
    if (!openFilesInSameWindow && !openFilesInNewWindow) {
      ...
      if (isFirstWindow && directoriesToOpen.length && filesToOpen.length) {
        ...
        for (const item of windowList) {
          ...
          this._createEditorWindow(path.dirname(fileList[0]!), fileList)
        }
      }
      ...
      for (const pathname of filesToOpen) {
        this._createEditorWindow(path.dirname(pathname), [pathname])
      }
  ...
  private _listenForIpcMain(): void {
    ...
    onInternalChannel('app-open-file-by-id', (windowId: number, filePath: string) => {
      ...
      if (openFilesInNewWindow) {
        this._createEditorWindow(path.dirname(filePath), [filePath])
      } else {
        const editor = this._windowManager.get(windowId) as EditorWindow | undefined
        if (editor) {
          if (!editor.openedRootDirectory) {
            editor.openFolder(path.dirname(filePath))
          }
    ...
    onInternalChannel('app-open-files-by-id', (windowId: number, fileList: string[]) => {
      const openFilesInNewWindow = this._accessor.preferences.getItem<boolean>('openFilesInNewWindow')
      if (openFilesInNewWindow) {
        this._createEditorWindow(fileList[0] ? path.dirname(fileList[0]) : null, fileList)
      } else {
        const editor = this._windowManager.get(windowId) as EditorWindow | undefined
        if (editor) {
          if (!editor.openedRootDirectory && fileList[0]) {
            editor.openFolder(path.dirname(fileList[0]))
          }
    ...
    onInternalChannel('app-refresh-directory-by-id', (windowId: number) => {
      const editor = this._windowManager.get(windowId) as EditorWindow | undefined
      editor?.refreshFolder()
    })
    ...
    ipcMain.on('mt::open-file-by-window-id', (_e, windowId: number, filePath: string) => {
      ...
      const openFilesInNewWindow = this._accessor.preferences.getItem<boolean>('openFilesInNewWindow')
      if (openFilesInNewWindow) {
        this._createEditorWindow(path.dirname(resolvedPath), [resolvedPath])
      } else {
        const editor = this._windowManager.get(windowId) as EditorWindow | undefined
        if (editor) {
          if (!editor.openedRootDirectory) {
            editor.openFolder(path.dirname(resolvedPath))
          }
