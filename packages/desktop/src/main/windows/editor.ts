...
class EditorWindow extends BaseWindow {
  ...
  createWindow(
    ...
    // A window without a project is a project picker, never an untitled editor.
    // Project-backed new files are created by the renderer directly on disk.
  ...
  openUntitledTab(selected: boolean = true, markdown: string = ''): void {
    // TODO: Don't allow new files if quitting.
    if (this.lifecycle === WindowLifecycle.QUITTED || !this._openedRootDirectory) return
  ...
  /** Rebuild the current directory watcher so it emits a fresh tree snapshot. */
  refreshFolder(): void {
    if (
      this.lifecycle !== WindowLifecycle.READY ||
      !this._openedRootDirectory ||
    ...
    const pathname = this._openedRootDirectory
    ipcMain.emit('watcher-unwatch-directory', this.browserWindow, pathname)
    ...
    try {
      ...
      const rootDirectory = restoredRootInfo?.isDir
        ...
        : fallbackRootInfo?.isDir
          ? fallbackRootInfo.path
          : null

      if (!rootDirectory) {
        browserWindow!.webContents.send('mt::load-state', {
          editor: { currentFileId: null, tabs: [], restoreWarnings: [] },
          project: null,
          layout: bufferState.layout ?? null
        })
        return
      }
      this.openFolder(rootDirectory)
      bufferState.project = { rootDirectory }

      // Older sessions may contain pathless untitled buffers. Preserve their
      // content while upgrading them to real files inside the restored project.
      let untitledIndex = 1
      for (const tab of bufferState.tabs) {
        if (tab.pathname) continue

        let filename = `Untitled-${untitledIndex}.md`
        let pathname = path.join(rootDirectory, filename)
        while (fs.existsSync(pathname)) {
          untitledIndex += 1
          filename = `Untitled-${untitledIndex}.md`
          pathname = path.join(rootDirectory, filename)
        }
        fs.writeFileSync(pathname, tab.markdown ?? '', 'utf8')
